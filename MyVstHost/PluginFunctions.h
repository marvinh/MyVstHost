...
#ifndef PluginFunctions_h
#define PluginFunctions_h
...
struct PluginLoader
{
    ...
    bool loadLibrary (const char* fileName)
    {
        ...
        if (url == 0)
            return false;
        module = CFBundleCreate (kCFAllocatorDefault, url);
        CFRelease (url);
...
#if _WIN32
        mainProc = (PluginEntryProc)GetProcAddress ((HMODULE)module, "CFBundleMain");
        if (!mainProc)
...
void checkEffectProcessing (AEffect* effect)
{
    ...
    for (VstInt32 processCount = 0; processCount < kNumProcessCycles; processCount++)
    {
        ...
        effect->processDoubleReplacing (effect, (double**)inputs,(double**) outputs, kBlockSize);
